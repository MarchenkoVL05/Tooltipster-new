	...
	Plugin.prototype = {
		...
		_init: function() {
			...
			if (document.querySelector) {
				...
				if (self.$el.getAttribute('data-tooltipster-initialTitle') === undefined) {

					initialTitle = self.$el.getAttribute('title');

					// we do not want initialTitle to have the value "undefined" because of how jQuery's .data() method works
					if (initialTitle === undefined) initialTitle = null;

					self.$el.setAttribute('data-tooltipster-initialTitle', initialTitle);
				}
				...
				if ((!deviceHasTouchCapability && self.options.iconDesktop) || (deviceHasTouchCapability && self.options.iconTouch)) {
					...
					if(typeof self.options.icon === 'string'){
						...
						self.$elProxy.innerHTML = '<span class="'+ self.options.iconTheme +'"></span>';
						self.$elProxy.textContent = self.options.icon;
					}
					// if it is an object (sensible choice)
					else {
						// (deep) clone the object if iconCloning == true, to make sure every instance has its own proxy. We use the icon without wrapping, no need to. We do not give it a class either, as the user will undoubtedly style the object on his own and since our css properties may conflict with his own
						if (self.options.iconCloning) self.$elProxy = self.options.icon.cloneNode(true);
				...
				if (self.options.trigger == 'hover') {
					...
					if (deviceHasTouchCapability && self.options.touchDevices) {
						...
						self.$elProxy.addEventListener('touchstart', function() {
							self._showNow();
						});
					}
				}
					...
					self.$elProxy.addEventListener('click', function() {
						if (!deviceIsPureTouch() || self.options.touchDevices) {
							self._show();
						}
					});
				...
				if (self.enabled && self.Content !== null) {
					...
					if (self.options.onlyOne) {
						...
						Array.prototype.forEach.call(notSelfElArray, function(el, i) {
							...
							Array.prototype.forEach.call(nss, function(ns, i){
								var instance = $el.getAttribute('data-' + ns),
					...
					var finish = function() {
						...
						Array.prototype.forEach.call(self.callbacks.show, function(c,i) { c.call(self.$el); });
						self.callbacks.show = [];
					};
						...
						if (supportsTransitions()) {
							...
							setTimeout(function() {
								...
								if(self.Status != 'hidden'){
									...
									self.$tooltip.classList.remove('tooltipster-content-changing');
									...
									setTimeout(function() {
										...
										if(self.Status !== 'hidden'){	
											...
											self.$tooltip.msTransition = self.options.speed + 'ms';
											self.$tooltip.transition = self.options.speed + 'ms';
										}
									}, self.options.speed);
								}
							}, self.options.speed);
		...
		hide: function(callback) {
			...
			var finishCallbacks = function() {
				...
				Array.prototype.forEach.call(self.callbacks.hide, function(c,i) { c.call(self.$el); });
				self.callbacks.hide = [];
			};
		...
		reposition: function() {
			...
			if (document.querySelector('body').querySelector(self.$tooltip).length !== 0) {
				...
				if (self.$elProxy.matches('area')) {
					var areaShape = self.$elProxy.getAttribute('shape'),
						mapName = self.$elProxy.parentNode.getAttribute('name'),
						map = $('img[usemap="#'+ mapName +'"]'),
						mapOffsetLeft = map.getBoundingClientRect().left,
						mapOffsetTop = map.getBoundingClientRect().top,
						areaMeasurements = self.$elProxy.getAttribute('coords') !== undefined ? self.$elProxy.getAttribute('coords').split(',') : undefined;
				...
				function dontGoOffScreenY(switchTo, switchFrom) {
					...
					if(((proxy.offset.top - document.scrollTop - tooltipHeight - offsetY - 12) < 0) && (switchFrom.indexOf('top') > -1)) {
						practicalPosition = switchTo;
					}

					// if it goes off the bottom of the page
					if (((proxy.offset.top + proxy.dimension.height + tooltipHeight + 12 + offsetY) > (document.scrollTop + window.innerHeight)) && (switchFrom.indexOf('bottom') > -1)) {
						practicalPosition = switchTo;
				...
				self.$tooltip.style.top = Math.round(myTop) + 'px';
				self.$tooltip.style.left = Math.round(myLeft) + 'px';
			}
		...
		destroy: function() {
			...
			if(ns.length === 1){
				...
				if (self.options.restoration === 'previous'){
					title = self.$el.getAttribute('data-tooltipster-initialTitle');
				}
					...
					title =
						...
						self.Content :
						document.createElement('div').appendChild(self.Content).innerHTML;
				}

				if (title) {
					self.$el.setAttribute('title', title);
				}

				// final cleaning
				self.$el.classList.remove('tooltipstered');
				self.$el.removeAttribute('data-tooltipster-ns');
				self.$el.removeAttribute('data-tooltipster-initialTitle');
			}
			else {
				...
				self.$el.setAttribute('data-tooltipster-ns', ns);
			}
